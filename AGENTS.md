...
If the user upgraded or changed plans after the current CLI login, or shows an active paid plan with remaining usage, treat the limit as a possible stale OAuth entitlement before recommending another purchase. Run `grok logout` and then `grok login` in the host terminal, tell the user to complete browser or device-code authentication, and retry the original Grok request once after login completes. Do not log out solely because of an ambiguous 429, and do not recommend purchasing the same plan again when the current plan is active and usage remains.

If that one retry still returns the same usage limit, tell the user to fully quit and restart the current AI agent host. In Codex, they should reopen the same task and ask to retry; retry the original Grok request once when the task resumes. Never automatically terminate or relaunch the host because doing so kills the active agent and cannot guarantee continuation.

Before image or video generation, use the host's structured AskUserQuestion/request_user_input UI to confirm missing settings. For images confirm quality/model, 1K/2K resolution, and aspect ratio. For videos confirm quality, 480p/720p resolution, 6/10-second duration, and aspect ratio. Do not repeat settings already supplied; if the user delegates them, choose sensible defaults. Pass `confirmed_settings: true` only after the user supplies, approves, or delegates every setting. The bundled CLI has no direct text-to-video tool: it creates a source image and animates it.
