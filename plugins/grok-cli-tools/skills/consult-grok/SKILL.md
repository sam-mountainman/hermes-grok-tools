...
If a tool returns `error_type: usage_limit`, show its complete error message without removing or rewriting the SuperGrok upgrade link. The user-facing response must include the clickable `[SuperGrokプランを確認・アップグレード](https://grok.com/supergrok?referrer=pricing&target=supergrok)` link and the original Grok CLI error. Do not retry automatically or silently reduce reasoning quality.
