...
If the retry still returns the same usage limit, tell the user to fully quit and restart the current Gemini or Antigravity host, then retry once when the conversation resumes. Never automatically terminate or relaunch the host because the active agent cannot reliably continue afterward.
